...
> ✅ **L2 跨类型（X 组）全部完成**（2026-08-24）：X-1~X-5 五项均已实现并回填（5466170 / 0cdfa43 / fb95e3a / 2e62d80+176d8d6 / 5124d91），详见下方 X 组表格。至此 **L2 全部清零**。
...
| 状态 | 项 | 目标类型 · 优先级 | 要点 |
|---|---|---|---|
| ✅ | X-1 睡眠定时 | 漫画 · 中 | 按分钟（15/30/45/60/90/自定义）+ 按话数（1-3）双模式。`ComicSleepTimerState` 会话级载体 + 设置面板「睡眠定时」分组（5466170）；阅读器持有 Timer：分钟到时暂停并停止自动翻页/自动滚动，话数模式在进入新章节（整章加载/seam/paged 重锚）递减归零触发；首次加载不计数 |
| ✅ | X-2 跨作品待读队列 | 小说/漫画 · 中 | `ReadingQueueStore` 复用 `PlayQueueStore` 持久化模式（SharedPreferences `reading_queue_v1`/`_current_v1`，仅存身份+起始章，打开时按源重抓目录）；队列弹层（点击打开自动移出/长按移除/清空）；入口：小说/漫画阅读器三点菜单 + 书架收藏长按菜单；`fetchNovelChapters`/`fetchChapters` 按模块路由打开（0cdfa43）+ 8 项单测 |
| ✅ | X-3 统一图片收藏图库 | 播放器/小说 · 低-中 | `ImageFavorite` 增加 source 维度（comic/player/novel），漫画条目维持原存储键兼容存量、截图/插图按 `来源::img::URL` 去重；`toggleByUrl`；图库缩略图来源角标；播放器截图保存后 SnackBar「收藏到图库」动作；小说插图大图查看器收藏按钮（fb95e3a） |
| ✅ | X-4 阅读中预下载后续章节 | 小说 · 中 | `NovelPreDownloadPreferences`（开关/阈值%/数量持久化）+ `NovelPreDownloader`：进度越过阈值后后台串行抓取后续 N 章正文，内存 + Hive `novel_pre_downloads` 持久化，切章优先命中缓存（离线可读）；触发点分页/滚动进度回调（每章一次）；全局阅读设置「预下载」分区（2e62d80，缓存读取恢复 176d8d6） |
| ✅ | X-5 朗读通知栏控制 | 小说 TTS · 低-中 | `AudioPlaybackSession` position/duration 流可空（无进度会话不显示进度条）；TTS 朗读中 attach（标题随章节刷新）、暂停保持会话、停止/退出 detach；通知栏播放/暂停/上句/下句；播放状态流按 isPlaying 变化广播（5124d91） |
