...
| 播放器 | 0 待办（9 项全部完成，F-24 归档） | 5 | 1（F-13 归档）+ F-24 归档 + 归档 3 项 |
| 漫画 | 9 | 6 | 5 + 暂缓清单 |
...
| ❌ | 桌面真 PiP 窗口 | F-24 | multi-window + GlobalKey reparent 播放子树（不重建状态），固定宽高比置顶——**已归档：desktop_multi_window 在 Windows 上因 MissingPluginException 不可用，全部走系统 PiP（F-23）** |
