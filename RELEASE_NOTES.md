...
> ⚠️ **v0.4.4 是 v0.4.3 的跟进版**（UI 设计基准落地：弹层限高合规化、语义状态色 token 化、间距/圆角统一收敛；五项功能修复：小说目录实时同步、本地导入漫画、本地文件管理、下载路径选择器防冻结、RSS 订阅优化；并恢复误删的平台工程文件）。**仍属 0.4.x 预览版，Bug 数量可能高于稳定版**。追求稳定请留在 v0.3.1，愿意试用请先备份数据。详见下方《更新日志（v0.4.3 → v0.4.4）》。
