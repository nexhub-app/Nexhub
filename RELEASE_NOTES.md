...
> ✅ **NexHub 基础功能已完成**：四合一聚合、源即插件、Bangumi 同步、网络配置、收藏分组评论、阅读统计、调试崩溃日志等核心能力均已就绪，**可作为稳定版长期使用、放心升级**。本版新增完整的**本地内容导入体系**（EPUB / PDF / CBR / RAR / 7Z 等）。无破坏性数据变更，可直接覆盖安装。详见下方《更新日志（v1.0.0 → v1.1.0）》。
...
- **本地内容导入**（v1.1.0 新增）：小说支持 TXT / EPUB，漫画支持 PDF / CBR / RAR / 7Z / ZIP / CBZ 等归档与整文件夹导入（Android 支持 SAF 文件夹）；书架一键导入，离线也能看。
