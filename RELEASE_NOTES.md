...
- **安卓 APK 签名密钥（RSA 4096，经 CI Secret 注入）**：从本次发布起，release 包改用**强度更高的 RSA 4096 签名密钥**，且密钥文件不再提交入库（通过 GitHub Actions Secret 注入，符合生产级安全要求）。**注意：因本次更换了签名密钥，从使用旧密钥签名的旧版本升级到新版本时无法覆盖安装，需先卸载旧版再安装**；之后各新版本之间可正常覆盖升级。若要上架应用商店，请改用你自己的私有正式签名密钥。
