...
本版重点两大块：**UI 设计基准落地**（弹层限高合规化、语义状态色 token 化、间距 / 圆角统一收敛）与 **五项功能修复**（小说目录实时同步、本地导入漫画、本地文件管理、下载路径选择器防冻结、RSS 订阅优化）；同时恢复上一版误删的平台工程文件。
...
- **UI 优化八项**：窄屏标签隐藏图标只留文字、源管理内层 Tab 可滚动；源列表「未登录」徽章；播放方向默认横屏（桌面 / 平板不强制转向）；播放器 9 个弹层合规化 + 发弹幕横竖屏适配 + 媒体信息 URL 复制按钮 + 播放统计标签可换行；全屏退出残留修复（`edgeToEdge` 统一）；小说主题跟随三档（跟随应用 / 始终夜间 / 始终日间）。
