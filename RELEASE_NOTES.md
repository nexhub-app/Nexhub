...
> ✅ **NexHub 基础功能已完成**：四合一聚合、源即插件、Bangumi 同步、网络配置、收藏分组评论、阅读统计、调试崩溃日志等核心能力均已就绪。**从本版起可作为稳定版长期使用、放心升级**。无破坏性数据变更，可直接覆盖安装。详见下方《更新日志（v0.5.1 → v1.0.0）》。

> 📌 **关于后续更新**：作者将在 **3 天后** 对底层框架做一次较大升级，升级期间可能较长时间不发布新版本；**3 天内（框架升级前）反馈的 bug 会及时修复**。框架升级完成后恢复正常迭代。
