...
> ⚠️ **升级必读（Android）**：本次发布**更换了 APK 签名密钥**。如果你手机上已安装**更早的旧版本**（使用旧密钥签名），**无法直接覆盖升级**——安装新 APK 时会提示「签名不一致 / 应用未安装」或安装失败。请先**卸载旧版**再安装本版本。卸载一般不会删除你的源配置与下载内容；如担心数据，可先手动备份手机上的 `NexHub` 目录。
